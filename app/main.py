...
async def find_best_team(api_key: str, name: str, league: Optional[int], season: int):
    ...
    target = normalize_team_name(original)

    def score_item(item):
        team_name = item.get("team", {}).get("name", "")
        country = (item.get("team", {}).get("country") or "").lower()
    ...
    found_items.sort(key=score_item, reverse=True)
    best = found_items[0]
...
async def search_teams(
    ...
    team = await find_best_team(api_key, name, league, season)
    if not team:
...
async def analyze_by_name(
    ...
    home_team = await find_best_team(api_key, home_name, league, season)
    if not home_team:
        return {"ok": False, "detail": f"Tým nenalezen: {home_name}"}

    away_team = await find_best_team(api_key, away_name, league, season)
    if not away_team:
        return {"ok": False, "detail": f"Tým nenalezen: {away_name}"}
